...
def parse_frontmatter_md(path):
    """Load the cornerstone article from Markdown frontmatter and preserve its authored structure."""
    text = Path(path).read_text(encoding="utf-8").replace("\r\n", "\n").strip()
    if not text.startswith("---\n"):
        raise ValueError("Missing frontmatter in cornerstone article")
    _, fm_text, md = text.split("---\n", 2)
    meta = {}
    ...
    def flush_faq():
        nonlocal current_faq_q, current_faq_a
        if current_faq_q is not None:
            faq.append((current_faq_q, current_faq_a or ""))
            current_faq_q = current_faq_a = None
    ...
    while i < len(lines):
        ...
        if line.startswith('## '):
            ...
            continue
        if line.startswith('### '):
            # Preserve subheadings inside the parent section as HTML blocks.
            if current is not None:
                current['blocks'].append(('subheading', line[4:].strip()))
            i += 1
            continue
        if line.startswith('![') and '](' in line and line.endswith(')'):
            m = re.match(r'!\[([^]]+)\]\(([^)]+)\)', line)
            if current is not None and m:
                current['blocks'].append(('image_path', m.group(2), m.group(1)))
            i += 1
            continue
        if line.startswith('| ') and i+1 < len(lines) and lines[i+1].startswith('|---'):
            ...
            while i < len(lines) and lines[i].startswith('|'):
                ...
                i += 1
            if current is not None:
                current['blocks'].append(('table', header, rows))
            continue
        if line.startswith('- '):
            items = []
            while i < len(lines) and lines[i].startswith('- '):
        ...
        if re.match(r'^\d+\. ', line):
            ...
            continue
        if line.startswith('**') and line.endswith('**') and current and current['title'] == 'Frequently Asked Questions':
            flush_faq()
            current_faq_q = line[2:-2].strip()
            i += 1
            while i < len(lines) and not lines[i].strip():
                i += 1
            if i < len(lines) and lines[i].strip() and not lines[i].startswith('**'):
                current_faq_a = lines[i].strip()
                i += 1
            continue
        if not line.strip() or line.strip() == '---':
            i += 1
            continue
        # Gather a paragraph, except FAQ answers are handled above.
        para = [line.strip()]
        i += 1
        while i < len(lines) and lines[i].strip() and not lines[i].startswith(('## ','### ','> **[IMAGE','- ','| ','**')) and not re.match(r'^\d+\. ', lines[i]):
            para.append(lines[i].strip())
            i += 1
        if current is not None:
            current['blocks'].append(('p', ' '.join(para)))
        else:
            intro.append(('p', ' '.join(para)))

    flush_faq()
    # Remove FAQ heading content blocks: the FAQ section gets a dedicated rendered structure.
    for sec in sections:
        if sec['title'] == 'Frequently Asked Questions':
            sec['faq'] = faq
            sec['blocks'] = []

    return meta, sections, intro


def load_cornerstone_article():
    path = os.path.join(ROOT, 'content', 'articles', 'what-is-defi.md')
    meta, sections, intro = parse_frontmatter_md(path)
    return dict(
        slug=meta['slug'], title=meta['title'], cat='defi', desc=meta['metaDescription'],
        read='10 min', keywords='what is defi decentralized finance basics', sections=sections, intro=intro,
        takeaways=[
            'DeFi replaces financial intermediaries with open smart contracts',
            'Lending, DEXs, stablecoins and staking are core DeFi applications',
            'Open access and transparency come with significant technical and market risks',
            'Self-custody makes security practices essential',
        ],
        author_key='research', type=meta.get('type','Cornerstone'), pillar=meta.get('pillar'),
        date=meta['datePublished'], dateModified=meta['dateModified'],
        metaTitle=meta['metaTitle'], metaDescription=meta['metaDescription'],
        focusKeyword=meta.get('focusKeyword','what is defi'), featuredImage=meta['featuredImage'],
        faq=next((sec.get('faq', []) for sec in sections if sec['title'] == 'Frequently Asked Questions'), [])
    )

ARTICLES = [
# ---------------- DeFi ----------------
load_cornerstone_article(),
...
def seo_head(path, title, desc, breadcrumbs, article=None, profile=None):
    ...
    if article and article.get("featuredImage"):
        image = "https://cryptostackhub.com" + article["featuredImage"]
    crumbs_json = []
    ...
    graph = [
        {"@type":"Organization","@id":"https://cryptostackhub.com/#organization","name":"CryptoStackHub","url":"https://cryptostackhub.com/","logo":{"@type":"ImageObject","url":"https://cryptostackhub.com/assets/images/cryptostackhub-logo.png"},"image":"https://cryptostackhub.com/assets/images/og-default.png"},
    ...
    if article:
        ...
        graph.append({"@type":"Article","@id":canonical+"#article","mainEntityOfPage":{"@type":"WebPage","@id":canonical},"headline":article["title"],"description":article.get("metaDescription",article["desc"]),"datePublished":article["date"],"dateModified":article.get("dateModified",article["date"]),"author":{"@type":"Organization","@id":"https://cryptostackhub.com"+author_url(article["author_key"])+"#organization","name":author["name"],"url":"https://cryptostackhub.com"+author_url(article["author_key"])},"publisher":{"@id":"https://cryptostackhub.com/#organization"},"image":[image],"articleSection":article.get("pillar", ""),"keywords":article.get("focusKeyword", "")})
...
def post_card(a, featured=False):
    ...
    visual = f'<img class="card-feature-image" src="{a["featuredImage"]}" alt="{esc(a["title"])} featured illustration" loading="lazy" decoding="async">' if a.get("featuredImage") else f'<div class="pv-bg" style="{style}"></div><div class="pv-icon">{ICONS[a["cat"]]}</div>'
...
def feature_card(a):
    c = CATS[a["cat"]]
    visual = f'<img class="card-feature-image" src="{a["featuredImage"]}" alt="{esc(a["title"])} featured illustration" loading="lazy" decoding="async">' if a.get("featuredImage") else f'<div class="pv-bg" style="--tint2:{c["tint2"]};--tint3:{c["tint3"]}></div><div class="pv-icon">{ICONS[a["cat"]]}</div>'
...
def write(path, html):
    ...
    faq_schema = ""
    if article and article.get("faq"):
        faq_entities = [{"@type":"Question","name":q,"acceptedAnswer":{"@type":"Answer","text":re.sub(r"<[^>]+>", "", inline_markdown(ans))}} for q, ans in article["faq"]]
        faq_schema = '<script type="application/ld+json">' + json.dumps({"@context":"https://schema.org","@type":"FAQPage","mainEntity":faq_entities}, ensure_ascii=False, separators=(",", ":")) + '</script>'
    html = html.replace("</head>", seo + faq_schema + "</head>", 1)
...
def render_article(a, idx):
    c = CATS[a["cat"]]
    toc = "".join(f'<a href="#faq">{esc(s["title"])}</a>' if s["title"] == "Frequently Asked Questions" else f'<a href="#s{i}">{esc(s["title"])}</a>' for i, s in enumerate(a["sections"]))
    secs_html = []
    for i, s in enumerate(a["sections"]):
        if s["title"] == "Frequently Asked Questions":
            continue
        blocks = []
        for b in s["blocks"]:
            if isinstance(b, tuple):
                kind = b[0]
                if kind == "p":
                    blocks.append(f"<p>{inline_markdown(b[1])}</p>")
                elif kind == "subheading":
                    blocks.append(f"<h3>{inline_markdown(b[1])}</h3>")
                elif kind == "ul":
                    blocks.append("<ul>" + "".join(f"<li>{inline_markdown(x)}</li>" for x in b[1]) + "</ul>")
                elif kind == "ol":
                    blocks.append("<ol>" + "".join(f"<li>{inline_markdown(x)}</li>" for x in b[1]) + "</ol>")
                elif kind == "image_path":
                    blocks.append(f'<figure class="article-figure"><img src="{esc(b[1])}" alt="{esc(b[2])}" loading="lazy" decoding="async"></figure>')
                elif kind == "table":
                    head_cells = "".join(f"<th>{inline_markdown(x)}</th>" for x in b[1])
                    rows = "".join("<tr>" + "".join(f"<td>{inline_markdown(x)}</td>" for x in row) + "</tr>" for row in b[2])
                    blocks.append(f'<div class="table-wrap"><table><thead><tr>{head_cells}</tr></thead><tbody>{rows}</tbody></table></div>')
            elif b == "code":
    ...
    intro_html = "\n".join(f"<p>{inline_markdown(b[1])}</p>" for b in a.get("intro", []))
    body = intro_html + ("\n" if intro_html and secs_html else "") + "\n".join(secs_html)
    ...
    faq_pairs = next((sec.get("faq", []) for sec in a["sections"] if sec["title"] == "Frequently Asked Questions"), [])
    faq_html = ""
    if faq_pairs:
        faq_html = '<section class="article-faq"><h2 id="faq">Frequently Asked Questions</h2>' + "".join(f'<div class="faq-item"><h3>{inline_markdown(q)}</h3><p>{inline_markdown(ans)}</p></div>' for q, ans in faq_pairs) + '</section>'
...
<div class="wrap" style="margin-top:10px">
  <div class="article-featured"><img src="{a.get('featuredImage','')}" alt="Decentralized finance network with Ethereum, wallets, users and financial services connected across a global network" loading="eager" decoding="async"></div>
...
{newsletter()}
"""
    page_title = a.get("metaTitle", f"{a['title']} — CryptoStackHub")
    page_desc = a.get("metaDescription", a["desc"])
    page_title = a.get("metaTitle", f"{a['title']} — CryptoStackHub")
    page_desc = a.get("metaDescription", a["desc"])
    write(f"articles/{a['slug']}/index.html", page(page_title, page_desc, html))
...
def main():
    for i, a in enumerate(ARTICLES):
        a["date"] = a.get("date", DATES[i])
        a["dateModified"] = a.get("dateModified", a["date"])
    for i, a in enumerate(ARTICLES):
        render_article(a, i)
